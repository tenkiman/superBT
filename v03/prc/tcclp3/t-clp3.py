...
if(clipermodel == 'wpclpr' or clipermodel == 'swpclp' or clipermodel == 'seiclp'
   or clipermodel == 'oclip' ):
    ...
    tau=0
    print "lat lon %5.1f %6.1f tau: %03d"%(lat0,lon0,tau)
    for i in range(6):
        lat=lalo[2*i]
        lon=lalo[2*i+1]
        tau=tau+12
        print "lat lon %5.1f %6.1f tau: %03d"%(lat,lon,tau)
