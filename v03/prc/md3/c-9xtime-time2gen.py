    ...
    for basin in basins:
        ...
        for card in cards:
            ...
            if(find(card,'genDiff')):
                ...
                print 'qqqq',genDiff,stmNN,stm9X,b1idNN,ibasin,year
                ...
                print pNNmask
                pNNs=glob.glob(pNNmask)
                pNN=pNNs[0]
                pNNSav="%s-SAV"%(pNN)
                npNNSav=MF.getPathSiz(pNNSav)
                
                pNNBTmask="%s/%s/%s/%s*/%s?-sum-BT.txt"%(sbtSrcDir,year,ibasin,stmNN[0:2],stmNN[0:2])
                print pNNBTmask
                pNNBTs=glob.glob(pNNBTmask)
                pNNBT=pNNBTs[0]
