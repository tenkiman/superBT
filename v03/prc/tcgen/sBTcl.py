...
#ccccccccccccccccccccccccccccclass AdeckGen(Adeck):
...
class LsDiagFile(MFutils):
    ...
    def __init__(self,dtg,
        ...
        self.sbtProdDirTcd=sbtProdDirTcdiag
        self.verb=verb
        self.dobail=dobail

        self.year=dtg[0:4]
        
        wdir=sbtProdDirTcdiag
...
"""
        ...
        for dtg in self.tdtgs:
            ...
            rc=MF.ChkPath(zipPath)
            if(rc == 0):
                print 'WWW-AdeckGenTctrk2.initAdeckPathsZip zipPath: ',zipPath,' not there sayounara...press...'
                self.cards=cards
                return
...
class gXbasemap2(MFbase):
    ...
    def set(self,
        ...
        #
        obmdir=self.bmdir
        self.pngpath="%s/bm.%s.png"%(obmdir,self.bmname)
...
class Era5(Model2):
    ...
    def setprvarc(self,dtg=None,tau=None):
        ...
        if(tau == 0):
            prc='''(const(prc.2(t+1),0,-u)-const(prc.2(t-0),0,-u))'''
            modelprvar="""_prvarc='( ( %s )*4*1000)'"""%(prc)
            ...
            prc='''(const(prc.2(t+0),0,-u)-const(prc.2(t-2),0,-u))'''
            modelprvar="""_prvarc='(  ( %s )*2*1000)'"""%(prc)
...
"""
        ...
        (ctlpath,taus,nfields,tauOffset,sfcpath)=getCtlpathTaus(model,self.bdtg,verb=verb)

        self.ctlpath=ctlpath
        self.sfcpath=sfcpath
        self.taus=taus
    ...
    def getHashTrk(self,key):
        ...
        for stmid in stmids:
            ...
            if( (sttype == 'GTC' or sttype == 'GPT' or sttype == 'GGT' or sttype == 'GNT' or sttype == 'GMT')                            # fron gothit in setModelStateGen
                ...
                or (sttype == 'NNN')                           # outside gen latitudes
            ...
            if( sttype == 'NNN' ):   # genesis storm outside genesis latitutdes
                print 'WWW getHashTrk outside gen lats amodel: ',amodel,' stmid: ',stmid,' basin: ',basin,' tau: ',tau
    ...
    def getGAfromGaProc(self,
        ...
        if(self.overrideGA):
            ...
            if(self.sfcpath != None):
                ga("open %s"%(self.sfcpath))
                
            lsctl="%s/ls.1deg.ctl"%(sbtGeogDatDir)
            ga("open %s"%(lsctl))
            ...
            # -- reinit and close all files..
            #
            self.ga('reinit')
            self.ga('open %s'%(self.ctlpath))
            if(self.sfcpath != None):
                self.ga("open %s"%(self.sfcpath))
            lsctl="%s/ls.1deg.ctl"%(sbtGeogDatDir)
    ...
    def fldDiagTcGen(self,basin,
        ...
        for latlon in latlons:
            ...
            if(hasattr(self.m2,'setprvarc')):
                prvarc=self.m2.setprvarc(bdtg,gentau)
            else:
                if('prc' in ge.vars):
            ...
            pcao=pcao + ga.get.asum('maskout(pc,-ls.3(t=1))',ge)
            pao=pao + ga.get.asum('maskout(p,-ls.3(t=1))',ge)
            areao=areao + ga.get.asum('maskout(const(p,1),-ls.3(t=1))',ge)
    ...
    def w2PlotTcGenFld(self,
        ...
        #
        byear=self.bdtg[0:4]
        odir='%s/%s/%s'%(sbtProdDirTcgen,byear,self.bdtg)
        MF.ChkDir(odir,'mk')
...
p2=re(psl,2)
pl=maskout(lterp(p2,psl),lterp(ls.3(t=1)-0.5,psl))
...
class TcPrBasin(MFbase):
    ...
    def __init__(self,model,basin,bdtg,gentaus=None,prs=None,verb=0):
        ...
        tdir="%s/w2flds/%s/%s"%(abdirDtg,year,bdtg)
        MF.ChkDir(tdir,'mk')
        
        prpath="%s/prbasin.%s.%s.%s.pyp"%(tdir,basin,bdtg,model)
        if(verb):
            print 'TcGenPrPath: ',prpath,'<<<----->>>'
        self.prpath=prpath
...
class InvHash(Tcgen,DataSet):
    ...
    def printFinalModelState(self,mstmid,doprint=0,do24hCPS=1,undef999=-9999.0,verb=0):
