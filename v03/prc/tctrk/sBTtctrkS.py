...
class TmTrkSimple(MFbase):
    ...
    def __init__(self,
        ...
        self.quiet=quiet
        self.doTrackerOnly=doTrackerOnly
        self.stmopt=stmopt
...
"""
    ...
    def doCP(self,ropt=''):
        ...
        if(self.haveTcs):
            ...
            nall=len(self.tcVstmids)
            oall=len(self.ostmids)

            print 'tctrk.atcf.path:',self.otctrkpath
            print 'tctrk.sink.path:',self.otctrksinkpath
            print 'nall: ',nall,' oall: ',oall
            
            for n in range(0,oall):

                ostmid=self.ostmids[n] 
                ...
                print ostmid,self.dtg,'vmax: %-3d'%(vmax),'n:',n,'atcfpath: ',atcfpath,'sinkpath: ',sinkpath
                # -- do the cat
                #
                if(n == 0 and (orc == 1 and nl != 999)):
                    cmd="cat %s > %s"%(atcfpath,self.otctrkpath)
                    mf.runcmd(cmd,ropt)
                    cmd="cat %s > %s"%(sinkpath,self.otctrksinkpath)
                    mf.runcmd(cmd,ropt)
                elif(n > 0 and (orc == 1 and nl != -999) ):
                    cmd="cat %s >> %s"%(atcfpath,self.otctrkpath)
                    mf.runcmd(cmd,ropt)
                    cmd="cat %s >> %s"%(sinkpath,self.otctrksinkpath)
                    mf.runcmd(cmd,ropt)
                elif(nl == -999):
                    print 'WWW -- Missing tracker for dtg: ',self.dtg,' and stmid: ',ostmid
                elif(nl == -888):
                    print 'WWW -- Exception in getting ostmid in self.tcVstmids dtg: ',self.dtg,' and stmid: ',ostmid
                elif(nl == -777):
                    print 'WWW -- ostmid NOT in self.tcVstmids dtg: ',self.dtg,' and stmid: ',ostmid
                else:
                    print 'EEE -- doCP dtg: ',self.dtg,' stmid: ',ostmid,' sayounara'
                    sys.exit()
    ...
    def doTrk(self,
        ...
        if( ( (not(self.detTest) and not(self.detGribTest)) or self.override) and ropt != 'norun'):
            ...
            #
            if(self.regridGen == self.regridTracker and didGenGrib):
                cmd="cp %s %s"%(self.grb10path,self.grbpath)
                mf.runcmd(cmd)
            else:
                MF.sTimer('latstctrk')
        ...
        #
        if( ( didGenGrib == 0 and not(self.doTrackerOnly) and not(self.genTest) ) and 
            ( didTrkGrib == 0 and not(self.detTest) ) ):
            self.setStatus()
            print 'WWWW -- no gribs to do tracker...returning...'
            return
        ...
        if(self.haveTcs):
            ...
            # -- create accumulation files here...before cycling by storms...
            #
        ...
        # -- DDDOOOTTTRRRAAACCCKKEEERRR OOONNNLLLYYY
        #
        if(self.doTrackerOnly):
        ...
        for basin in tcgenBasins:
            ...
            if(self.haveTcs):
                if(hasattr(self,'tcVcards')):
                    ...
                    print 'tcV,makeTCvCards deprecated by Mdeck3...'
                    sys.exit()
...
%2d %7d'''%(fcmin,nn,self.taus[n]*60)
    ...
    def setFldVars(self):
        ...
        # -- convert to geopotential meters for ecmwf
        #
        if(self.model == 'era5' or self.model == 'ecm5'): zgmfact=1.0/gravity
