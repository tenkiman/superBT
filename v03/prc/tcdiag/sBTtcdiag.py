...
class TcDiagS(MFbase):
    ...
    def __init__(self,dtg,model,
        ...
        #/dat3/dat/sbt-v03/superBT-V03-dat/dat
        #tbdir='/dat3/dat/sbt-v03/dat/tcdiag'
        ...
        useADs=0
        if(trkSource == 'tmtrkN-sbt'):
            useADs=1
            trkSource=trkSource.split('-')[0]
        ...
        self.adeckOutdirLocal=None
        self.diagfileOutdirLocal=None
        ...
        #
        if(useADs):
            # -- 20230314 -- need to setup a merge of these to drive by dtg vice -S stmopt
            self.initADs(dtg,model,asdir=adeckSdir,verb=verb)
        else:
        ...
        if(ctlpath == None):
            ...
            # -- this automatically pulls the nearest model dtg
            #
            rc=getW2fldsRtfimCtlpath(model,dtg)
            ...
            if(model == 'era5' and doSfc):
                ...
                rc=getW2fldsRtfimCtlpath(model,dtg,doSfc=1)
                if(rc == None):
        ...
        #print 'tttttttttttttttttttttttttttt',self.targetTaus,self.mdtg,self.tauOffset,self.ctlpath,self.ctlpath2
        #sys.exit()
    ...
    def makePrPrcSfcData(self,ropt='',verb=1):
        
        emask="%s/%s.*.ctl"%(self.tdir,self.model)
        ectls=glob.glob(emask)
        if(len(ectls) != 1):
            print 'EEE - no ctls in emask: ',emask
            return
        
        ipath=ectls[0]
        (idir,ifile)=os.path.split(ipath)
        ofile="sfc-%s"%(ifile)
        cpath="%s/%s"%(idir,ofile)

        if(MF.getPathSiz(cpath) <= 0):
            cmd="cp %s %s"%(ipath,cpath)
            mf.runcmd(cmd)
        
        dfile=ofile.replace('.ctl','.dat')    
        dpath="%s/%s"%(idir,dfile)

        gspath="%s/g-pr.gs"%(idir)
        ...
        dset="dset ^"
        icards=open(cpath).readlines()
        
        ocards=[]
        octl=''
        for icard in icards:
            if(mf.find(icard,'dset')):
                octl=octl+"""dset ^%s
title %s sfc fields
"""%(dfile,self.dtg)
            elif(mf.find(icard,'def')):
                octl=octl+icard
...
prc            0 0 pr 6-h rate [mm/day]
endvars"""
            
        rc=MF.WriteString2Path(octl, cpath)
...
"""%(ipath,dpath)
    ...
    def setDiagPath(self,stmid,tdir=None,tbdir=None,diag=0,dobail=0):
        ...
        self.shipstxt=dpath.replace('tcdiag.%s'%(self.model),'ships-txt.%s'%(self.model))
        self.shipsadk=dpath.replace('tcdiag.%s'%(self.model),'ships-adk.%s'%(self.model))
        self.shipslog=dpath.replace('tcdiag.%s'%(self.model),'ships-log.%s'%(self.model))
        self.shipscrq=dpath.replace('tcdiag.%s'%(self.model),'ships-crq.%s'%(self.model))
    ...
    def putAdeckCards(self,stmid,verb=1,dobail=1):
        ...
        if(hasattr(self,'adeckOutdirLocal') and self.adeckOutdirLocal != None):
        ...
        if(hasattr(self,'finalDiagPath')):
            ...
            if(hasattr(self,'diagfileOutdirLocal') and self.diagfileOutdirLocal != None):
...
"""
    ...
    def initM2(self,dtg,model,verb=0,override=0):
    ...
    # -- 20230314 --initADs -- sssiiimmmppplllee
    #

    def initADs(self,dtg,model,
                asdir='/dat3/dat/sbt-v03/dat/adeck-dtg',verb=0):
        
        year=self.dtg[0:4]
        amask="%s/%s/%s/tctrk.atcf*%s*"%(asdir,year,self.dtg,self.atcfname)
        apaths=glob.glob(amask)
        smask="%s/%s/%s/tctrk.sink*%s*"%(asdir,year,self.dtg,self.atcfname)
        spaths=glob.glob(smask)
        
        if(verb):
            print 'sss',self.dstmids
            print 'aaa',self.atcfname
            print 'ddd',self.dtg
            print 'aaa',amask
            print 'aaa',apaths
            print 'sss',smask
        ...
        if(len(apaths) == 1):
            ...
            print "WWW-initADs-- no apath in amask: ",amask,'...try running the tracker...'
            sys.exit()
        ...
        self.spath=spath
        self.aid=self.adeckaid
        self.adeckaid=self.atcfname
        
        didADlongest=0
     
        if(verb):
            print 'FFFaaa',self.apath
            print 'FFFsss',self.spath
        
        self.aD=Adeck(self.apath,verb=verb,aliases=self.aidAliases)

        if(self.spath != None):
            self.aDS=AdeckSink(self.spath,verb=verb)
        else:
            self.aDS=None
    ...
    def makeTCmeta(self,taus=None,tdir='/tmp',verb=0):
        ...
        (rc,scard)=self.md3.getMd3StmMeta(stmid)
        stmname=rc[4]
...
%s"""%(meta,metas[otau])
    ...
    def setTCtracker(self,stmid,aidSource=None,aidnameOverride=None,maxtau=168,quiet=0,verb=0):
        ...
        def mergeAidBt(aidtrks,aidtaus,bttrk,bttaus):
            mtaus=mf.uniq(aidtaus1+bttaus)
            otrk={}
            otaus=[]
            for mtau in mtaus:
                try:
                ...
                if(verb): print 'mmmeee',mtau,atrk,btrk
                if(atrk != None):
                    otrk[mtau]=atrk
                elif(btrk != None):
                    otrk[mtau]=btrk
                else:
                    print 'prob in mergeAidBt for mtau:',mtau,'neither available sayounara...'
                    sys.exit()
        ...
        #
        def makeBTaidtrk(stmid,dtg,etau=24,dtau=6,verb=verb):

            def btcs2aidtrk(btcs,btdtg,verb=0):
            ...
            (rc,btcs)=self.md3.getMd3track(stmid)
        ...
        #
        (bttrk,bttaus)=makeBTaidtrk(stmid,self.dtg,verb=verb)
        ...
        if(self.aD != None):
            ...
            if(self.aidtaus != None):
                ...
                lasttau1=aidtaus1[-1]
                if(ntaus1 < nbttaus):
                    if(self.verb): print 'fffEEEwwwEEErrr',ntaus1,nbttaus
                    chkBT=2
            else:
                chkBT=1
            ...
            if(chkBT == 1):
                ...
                #
                if(verb): print 'III(setTCtracker) -- SUBSTITUTE BT for: ',self.stmid,
                self.aidtrk=bttrk
                self.aidtaus=bttaus
                usingBTs=1
            ...
            if(chkBT == 2):
                ...
                # -- bail point 2 merge aid bt taus
                #
                (aidtrks1,aidtaus1)=mergeAidBt(aidtrks1,aidtaus1,bttrk,bttaus)
                if(verb): print 'III(setTCtracker) -- MERGE BT into incomplete tracker for: ',self.stmid,' ... '
        ...
        if(usingBTs):
            ...
            self.FtrkSource[self.stmid]=self.trkSource
    ...
    def chKOutput(self,iV,dstmids=None,aidSource=None,rc=None,quiet=0):
        ...
        if(status == -1):
            ...
            if(quiet):
                print
                print '''IIIIIIIIIIII -- already run; use -O (override=1) to override'''
                print
    ...
    def setLatLonTimeByTau(self,dtg,model,stmid,tau,
        ...
        if(abs(latc) >= self.maxLatitude):
            print 'WWWWW-----ETETET -- too far poleward in setLatLonTimeByTau...'
            #return(-1)
    ...
    def setInitialGA(self,xlint=10,ylint=5,dobasemap=1,bmoverride=0):
        ...
        if(dobasemap):
            ...
            if(not(MF.ChkPath(bm.pngpath,verb=self.verb)) or bmoverride):
                bm.draw()
                bm.putPng()
...
'z300:getexpr:0:-999:-999:300 [m]',
        ...
        if(varSua == None):
            ...
            if(self.model == 'ecmt'):
                ...
                if(self.model == 'era5'):
                    if(self.doSfc):
                        # -- 20230324 -- mod lsdiag.x to read in but not use prc
                        #    lsdiag.x -> lsdiag-prc.x  lsdiag-pr.x is original
                        varSUVP=[
                            ...
                            'pr:getprvar:0:-999:-999:pr 6-h rate [mm/day]',
                            'prc:getprvar:0:-999:-999:pr 6-h rate [mm/day]',
                        ]
                    else:
                        varSUVP=[
...
class Oisst(DataSet,f77GridOutput):
    ...
    ddir="%s/ocean/oisst"%(W2BaseDirDat)
...
class Era5sst(DataSet,f77GridOutput):
    ...
    ddir="%s/ocean/era5"%(W2BaseDirDat)
    ddirO="%s/ocean/oisst"%(W2BaseDirDat)
    ...
    def __init__(self,ga,ge,dtg,
        ...
        if(len(fd.statuss[dtgE]) == 0):
            ...
            if(len(fd.statuss[dtgE12]) == 0):
                print 'EEEEEEE in Ecm5sst -- no data for dtgE: ',dtgE,' or dtgE12: ',dtgE12,' sayounara...'
                sys.exit()
            else:
...
class TcTrkPlot(DataSet):
    ...
    def __init__(self,tG,md3,stmid,zoomfact=None,verb=1,otau=48,override=0,
        ...
        if( not(hasattr(tG,'ga2')) ):
            gaP=GaProc(ctlpath="%s/dum.ctl"%(sbtGslibDir),Quiet=Quiet,Bin=self.Bin)
...
def runLsDiag(mfT,tG,ropt='',override=0,doAdeckOut=1):
    ...
    if(hasattr(tG,'oadeck') and doAdeckOut):
        oadeckPath=outpath.replace('tcdiag.%s'%(tG.model),'oadeck.%s'%(tG.model))
...
def runLgem(diagpath,tG,dtg,model,stmid,
...
##            atrk={}
    ...
    if(len(atrk) == 0):
        btcs=tG.btcs
        atrk=getAtrkFromStmid(btcs,stmid,dtg)
...
def getAtrkFromStmid(btcs,stmid,dtg):
