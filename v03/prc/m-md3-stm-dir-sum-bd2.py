...
class TmtrkCmdLine(CmdLine):
    ...
    def __init__(self,argv=sys.argv):
        ...
        self.options={
            ...
            'doBdeck2':         ['2',1,0,'default for this version'],
...
for year in years:
    ...
    for basin in basins:
        ...
        for sdir in sdirs:
            ...
            topt=''
            if(doTrk): topt='-T'
            if(doBdeck2): topt=''  # defautl for bd2 versions
            #print 'mmm',mpath,mpathBT,mpathMRG
            if(mpath != None):
                cmd="m-mdeck3-bd2.py -r %s %s %s"%(mpath,vopt,topt)
                #mf.runcmd(cmd,'quiet')
                mf.runcmd(cmd,ropt)
            
            if(mpathBT != None):
                cmd="m-mdeck3-bd2.py -r %s %s %s"%(mpathBT,vopt,topt)
                #mf.runcmd(cmd,'quiet')
                mf.runcmd(cmd,ropt)
