...
#pylibdir="%s/lib/python"%(w2.PrcBdirW2)
#sbtpyp='sbt-v01.pyp'
#sbtpyppath="%s/%s"%(pylibdir,sbtpyp)

#usePyp=0
#doPyp=0
#MF.sTimer('sbt-load')
#if(usePyp):
    #sbt=getPyp(sbtpyppath)
#else:
    #sbt=superBT(version,verb=verb)
    #if(doPyp): putPyp(sbt, sbtpyppath)
...
if(stmopt != None):
    ...
    # -- when using a list, select by var #
    
    nvars=range(1,19)
    nvars=[1]

    ovars=['bvmax','bspd','stmspd',
           'mvmax',
           'shrspd',
           'tpw','rh700',
           'oprc3','oprg3','opri3',
    ...
    rc=sbt.makeGaNonVDevTSDict(sbtTSNonDev,'non',ovars,verb=verb)
    print sbt.gactlPath
    rc=sbt.makeGaNonVDevTSDict(sbtTSDev,'dev',ovars,verb=verb)
    print sbt.gactlPath
