...
#
sMdesc=lsSbtVars(verb=1)
mD3desc=lsMd3Vars(verb=1)
mD3sum=lsMd3SumVars(verb=1)
...
if(stmopt != None):
    ...
    for stmid in stmids:
        ...
        rc=md3.getMd3track(stmid,dobt=dobt,verb=verb)
    
        if(rc[0] == None):
            print 'EEEEEEEEEEEEEEE dtgs missing for stmid:',stmid
            sys.exit()
        ...
        iokDtg=1
        for sdtg in sbtdtgs:
            if(not(sdtg in m3dtgs)):
                print 'sbt missing for: ',stmid,' dtg: ',sdtg
                iokDtg=0
                break
            else:
                sbd=sbtVar[sdtg]
                ...
                mblon=m3trk[sdtg][1]
                if(verb):
                    print 'for: ',stmid,sdtg,'SSS',sblat,sblon,'MMM',mblat,mblon
        if(iokDtg):
            print 'GGG dtgs sbt=md3 for: ',stmid
